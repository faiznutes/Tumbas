...
let OrdersService = class OrdersService {
    ...
    extractProductVariants(product) {
        if (!Array.isArray(product.variants))
            return [];
        return product.variants;
    }
    async reduceStock(tx, productId, quantity, variantKey) {
    ...
    async create(data) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (requestedItems.length === 0) {
                throw new common_1.BadRequestException('Produk pesanan tidak boleh kosong');
            }
            ...
            for (const item of mergedItems) {
                ...
                if (product.status !== 'AVAILABLE') {
                    throw new common_1.BadRequestException(`Produk ${product.title} tidak tersedia`);
                }
                const variants = this.extractProductVariants(product);
                ...
                if (item.selectedVariantKey && !selectedVariant) {
                    throw new common_1.BadRequestException(`Varian produk ${product.title} tidak valid`);
                }
                if (selectedVariant) {
                    if (Number(selectedVariant.stock || 0) < item.quantity) {
                        throw new common_1.BadRequestException(`Stok varian ${selectedVariant.label} untuk ${product.title} tidak mencukupi`);
                    }
                }
                else if ((product.stock || 0) < item.quantity) {
                    throw new common_1.BadRequestException(`Stok produk ${product.title} tidak mencukupi`);
                }
